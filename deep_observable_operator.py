...
def p_zT_g_y(y, a_list):
    p_zT1_un = 0
    p_zT0_un = 0
    p_obs_g_acts = p_obs_g_actions(y[0:-1], a_list[0:-1])
    for v_T in range(prod_pomdp.state_size):
        # print("A state Done")
        act_T = prod_pomdp.actions[a_list[-1]]
        o_T = y[-1]
        st_T = prod_pomdp.states[v_T]
        emiss_prob = prod_pomdp.emiss[st_T][act_T][o_T]
        p_vtp1_obs_g_acts = p_vtp1_obs_g_actions(v_T, y[0:-1], a_list[0:-1])
        p_vT_g_y_un = emiss_prob * p_vtp1_obs_g_acts / p_obs_g_acts
        if st_T in prod_pomdp.goals:
            p_zT1_un += p_vT_g_y_un
        else:
            p_zT0_un += p_vT_g_y_un
    sum = p_zT1_un + p_zT0_un
    p_zT1 = p_zT1_un / sum
    p_zT0 = p_zT0_un / sum
    return p_zT1, p_zT0
...
def main():
    ...
    M = 10  # number of sampled trajectories
